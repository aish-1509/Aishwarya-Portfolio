...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Handle hover on desktop nav items
  ...
  // Handle mouse leaving the nav area
  ...
  window.addEventListener('load', () => {
    handleScroll();
    handleActiveLink();
    // Initial position for the indicator
    setTimeout(() => {
      const activeLink = document.querySelector('.nav-link.active');
      updateIndicator(activeLink);
    }, 100);
...
// Enhanced Background Effects and Cursor
document.addEventListener('DOMContentLoaded', function() {
...
function initHeroEnhancements() {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    const layer1 = document.querySelector('.hero-bg-layer-1');
    const layer2 = document.querySelector('.hero-bg-layer-2');
    const layer3 = document.querySelector('.hero-bg-layer-3');
    const shapes = document.querySelectorAll('.floating-shape');
    
    if (layer1) {
      layer1.style.transform = `translate(${mouseX * 5}px, ${mouseY * 5}px)`;
    }
    if (layer2) {
      layer2.style.transform = `translate(${mouseX * 8}px, ${mouseY * 8}px) rotate(${mouseX * 2}deg)`;
    }
    if (layer3) {
      layer3.style.transform = `translate(${mouseX * 3}px, ${mouseY * 3}px)`;
    }
    
    shapes.forEach((shape, index) => {
      const intensity = (index + 1) * 0.5;
      shape.style.transform = `translate(${mouseX * intensity * 10}px, ${mouseY * intensity * 10}px)`;
    });
  });
  
  // Smooth scroll for hero CTA and scroll indicator
  const heroBtn = document.querySelector('.hero-cta-btn');
  const scrollIndicator = document.querySelector('.scroll-indicator');
  
  [heroBtn, scrollIndicator].forEach(element => {
    if (element) {
      element.addEventListener('click', (e) => {
        if (element.getAttribute('href') === '#projects' || element === scrollIndicator) {
          e.preventDefault();
  ...
  // Add magnetic effect to hero button
  const magneticBtn = document.querySelector('.hero-cta-btn');
  if (magneticBtn) {
    magneticBtn.addEventListener('mousemove', (e) => {
      ...
      magneticBtn.style.transform = `translateY(-3px) translate(${x * 0.1}px, ${y * 0.1}px)`;
    });
    
    magneticBtn.addEventListener('mouseleave', () => {
      magneticBtn.style.transform = 'translateY(-3px) translate(0px, 0px)';
    });
  }
}
...
function createFloatingParticles() {
  ...
  // Create sparse 200 particles with depth layers
  for (let i = 0; i < 200; i++) {
    const particle = document.createElement('div');
    const depth = Math.random() * 3; // 0-3 depth layers
    
    // Assign depth-based classes
    if (depth < 1) {
      particle.className = 'particle particle-near';
    } else if (depth < 2) {
      particle.className = 'particle particle-mid';
    } else {
      particle.className = 'particle particle-far';
    }
    
    // Store initial position and depth for parallax calculations
    ...
    particles.push({
      ...
      initialX,
      initialY,
      depth: depth + 0.5, // 0.5-3.5 for parallax intensity
      parallaxX: 0,
      parallaxY: 0
    });
  ...
  function updateParallax() {
    particles.forEach(p => {
      // Calculate parallax offset (1-3% based on depth)
      const parallaxIntensity = p.depth * 0.01; // 0.5-3.5% range
      const targetParallaxX = mouseX * parallaxIntensity * 100;
      const targetParallaxY = mouseY * parallaxIntensity * 100;
      
      // Smooth parallax transition
      p.parallaxX += (targetParallaxX - p.parallaxX) * 0.1;
      p.parallaxY += (targetParallaxY - p.parallaxY) * 0.1;
      
      // Apply transform
      p.element.style.transform = `translate(${p.parallaxX}px, ${p.parallaxY}px)`;
    });
...
function createCustomCursor() {
  ...
  const interactiveElements = document.querySelectorAll('a, button, .button, .shadow__btn, .download-label, .project-card, .upcoming-card, .journey-card, .example-2 .icon-content a, .category-label');
...
function addInteractiveGlowEffects() {
  const cards = document.querySelectorAll('.project-card, .journey-card, .upcoming-card');
...
gsap.from(".project-card", {
  ...
  gsap.utils.toArray(".certificate-icon").forEach((icon, i) => {
    gsap.from(icon, {
      scrollTrigger: {
        trigger: icon,
        start: "top 90%",
  ...
  // Leadership icons animation
  gsap.utils.toArray(".leadership-icon").forEach((icon, i) => {
    gsap.from(icon, {
      scrollTrigger: {
        trigger: icon,
        start: "top 90%",
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const criticalImages = [
    './assets/AbhijeetBhalePortfolio.jpeg',
    './assets/cursor.png'
  ];
